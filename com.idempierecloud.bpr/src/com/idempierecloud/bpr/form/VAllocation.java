...
public class VAllocation extends CustomForm {
	...
	/**	Logger			*/
	public static CLogger log = CLogger.getCLogger(VAllocation.class);
...
//	private int			i_multiplier = 10;
	...
	{		
        ...
        StringBuilder sql = new StringBuilder("SELECT ao.name, ao.ad_org_id, "//1-2
			+ "p.DateTrx,p.DocumentNo,p.C_Payment_ID,"  //  1..3
		...
		try
		{
			...
			while (rs.next())
			{
				...
				line.add(rs.getBigDecimal(8));      //  5/7-ConvAmt
				...
				line.add(Env.ZERO);					//  7/8-Payment
				line.add(rs.getString(12));			//  8/11-Description
//				line.add(rs.getBigDecimal(8));		//  7/10-Multiplier
				//
				data.add(line);
			}
		}
...
//		columnNames.add(" ");	//	Multiplier
	...
	public void setPaymentColumnClass(IMiniTable paymentTable, boolean isMultiCurrency)
	{
		...
		paymentTable.setColumnClass(i++, String.class, false);      //  9-Description
	...
	public Vector<Vector<Object>> getInvoiceData(boolean isMultiCurrency, Object date, IMiniTable invoiceTable)
	{
		...
		try
		{
			...
			while (rs.next())
			{
				...
				pp = new KeyNamePair(rs.getInt(5), rs.getString(4));
				line.add(pp);                       //  2-Value
				line.add(rs.getString(12));      	//  3-BPartner
				if (isMultiCurrency)
				{
					line.add(rs.getString(6));      //  4-Currency
					line.add(rs.getBigDecimal(7));  //  5-Orig Amount
				}
				...
				line.add(rs.getString(13));				    //  9/11-OverUnder
...
//		columnNames.add(" ");	//	Multiplier
	...
	public void setInvoiceColumnClass(IMiniTable invoiceTable, boolean isMultiCurrency)
	{
		...
		invoiceTable.setColumnClass(i++, String.class, true);			//	12-Description
