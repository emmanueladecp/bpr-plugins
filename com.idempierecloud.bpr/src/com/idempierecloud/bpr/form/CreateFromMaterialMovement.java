	...
	public boolean save(IMiniTable miniTable, String trxName) {
		...
		for (int i = 0; i < miniTable.getRowCount(); i++)
		{
			if (((Boolean)miniTable.getValueAt(i, 0)).booleanValue()) {
				...
				int m_locatorTo_ID = DB.getSQLValue(movement.get_TrxName(), "select m_locator_id from m_locator ml where m_locatortype_id in (select m_locatortype_id from m_locatortype ml2 where name='INTRANSIT') and m_warehouse_id = ?", M_WarehouseTo_ID);
