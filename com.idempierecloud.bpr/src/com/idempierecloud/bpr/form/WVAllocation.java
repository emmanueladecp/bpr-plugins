 ...
 */
public class WVAllocation extends VAllocation implements ValueChangeListener, WTableModelListener {
	...
	private void zkInit() throws Exception
	{
		...
		row.appendCellChild(organizationTargetLabel.rightAlign());
		ZKUpdateUtil.setHflex(organizationTargetPick.getComponent(), "true");
		row.appendCellChild(organizationTargetPick.getComponent(),2);
		organizationTargetPick.showMenu();
	...
	public void dynInit() throws Exception
	{
		...
		// Organization Target filter selection
		AD_Column_ID = COLUMN_C_PERIOD_AD_ORG_ID; //C_Period.AD_Org_ID (needed to allow org 0)
		MLookup lookupOrgTarget = MLookupFactory.get(Env.getCtx(), form.getWindowNo(), 0, AD_Column_ID, DisplayType.TableDir);
		organizationTargetPick = new WTableDirEditor("AD_OrgTarget_ID", true, false, true, lookupOrg);
		organizationTargetPick.setValue(Env.getAD_Org_ID(Env.getCtx()));
		organizationTargetPick.addValueChangeListener(this);
	...
	public void valueChange (ValueChangeEvent e)
	{
		...
		// Organization Target
				if (name.equals("AD_OrgTarget_ID"))
				{
					m_AD_OrgTarget_ID = ((Integer) value).intValue();
