...
public class BPAllocation extends CustomForm {
	...
	/**	Logger			*/
	public static CLogger log = CLogger.getCLogger(BPAllocation.class);
...
//	private int			i_multiplier = 10;
	...
	{		
		...
		StringBuilder sql = new StringBuilder("SELECT ao.name, ao.ad_org_id, "//1-2
			...
			+ "p.MultiplierAP "
			+ ",bp.Name, ba.name as bankaccount " // 10
			+ ", p.description "
...
//			sql.append(" AND p.AD_Org_ID=" + m_AD_Org_ID);
		...
		try
		{
			...
			while (rs.next())
			{
				...
				line.add(rs.getString(11));			//	4-BPartner
				if (isMultiCurrency)
				{
					line.add(rs.getString(6));      //  5-Currency
					line.add(rs.getBigDecimal(7));  //  6-PayAmt
				}
				line.add(rs.getBigDecimal(8));      //  4/6-ConvAmt
				...
				line.add(rs.getString("description"));//  8/10-BankAccount
...
//		columnNames.add(" ");	//	Multiplier
	...
	public void setPaymentColumnClass(IMiniTable paymentTable, boolean isMultiCurrency)
	{
		...
		paymentTable.setColumnClass(i++, Timestamp.class, true);        //  2-TrxDate
		paymentTable.setColumnClass(i++, String.class, true);           //  3-Value
		paymentTable.setColumnClass(i++, String.class, true);       	//  4-BPartner
		if (isMultiCurrency)
		{
			paymentTable.setColumnClass(i++, String.class, true);       //  5-Currency
			paymentTable.setColumnClass(i++, BigDecimal.class, true);   //  6-PayAmt
		}
		paymentTable.setColumnClass(i++, BigDecimal.class, true);       //  7-ConvAmt
		paymentTable.setColumnClass(i++, BigDecimal.class, true);       //  8-ConvOpen
		paymentTable.setColumnClass(i++, BigDecimal.class, false);      //  9-Allocated
		paymentTable.setColumnClass(i++, String.class, false);    	    //  10-BankAccount
		paymentTable.setColumnClass(i++, String.class, false);    	    //  11-Description
//		paymentTable.setColumnClass(i++, BigDecimal.class, true);      	//  10-Multiplier
	...
	public Vector<Vector<Object>> getInvoiceData(boolean isMultiCurrency, Object date, IMiniTable invoiceTable)
	{
		...
		try
		{
			...
			while (rs.next())
			{
				...
				pp = new KeyNamePair(rs.getInt(5), rs.getString(4));
				line.add(pp);                       //  3-Value
				line.add(rs.getString(12));      	//  4-BPartner
				if (isMultiCurrency)
				{
					line.add(rs.getString(6));      //  5-Currency
					line.add(rs.getBigDecimal(7));  //  6-Orig Amount
				}
				line.add(rs.getBigDecimal(8));      //  6/7-ConvAmt
...
//		columnNames.add(" ");	//	Multiplier
	...
	public void setInvoiceColumnClass(IMiniTable invoiceTable, boolean isMultiCurrency)
	{
		...
		invoiceTable.setColumnClass(i++, String.class, true);       	//  4-BPartner
		if (isMultiCurrency)
		{
			invoiceTable.setColumnClass(i++, String.class, true);       //  5-Currency
			invoiceTable.setColumnClass(i++, BigDecimal.class, true);   //  6-Amt
		}
		...
		invoiceTable.setColumnClass(i++, BigDecimal.class, false);      //  9-Conv Discount
		invoiceTable.setColumnClass(i++, BigDecimal.class, false);      //  10-Conv WriteOff
		invoiceTable.setColumnClass(i++, BigDecimal.class, false);      //  11-Conv OverUnder
		invoiceTable.setColumnClass(i++, BigDecimal.class, true);		//	12-Conv Applied
		invoiceTable.setColumnClass(i++, String.class, true);			//	13-Description
