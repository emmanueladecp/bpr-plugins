...
public class MCostDetail extends X_M_CostDetail
{
		...
		int Org_ID, int M_ASI_ID)
	{
		...
		history.set_ValueOfColumn("AD_Client_ID", cost.getAD_Client_ID());
			...
			|| getPP_Cost_Collector_ID() != 0)
		{
			...
			boolean adjustmentProd = false;
			if (!addition && getM_ProductionLine_ID()>0) {
				MProductionLine prodLine = new MProductionLine(getCtx(), getM_ProductionLine_ID(), get_TrxName());
				if (prodLine.get_ValueAsString("JenisProduk").equals("T") && qty.signum()<0) {
					adjustmentProd = true;
				}
			}
			//
