		...
		if(reject.compareTo(BigDecimal.ZERO)>0) {
			...
			for(MInOutLineConfirm line: confirm.getLines(false)) {
				if(line.getDifferenceQty().compareTo(BigDecimal.ZERO)>0) {
					...
					int MLocator_Retur = DB.getSQLValue(line.get_TrxName(), "Select coalesce(min(M_Locator_ID),0)  from M_Locator where isactive='Y' AND M_LocatorType_ID=1000004 and M_Warehouse_ID=?", shipment.getM_Warehouse_ID());
					if(MLocator_Retur>0)
