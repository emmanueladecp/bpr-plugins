 ...
 *****************************************************************************/
package com.idempierecloud.bpr.acct;
			...
			for (int i = 0; i < p_lines.length; i++)
			{
				...
				if (!isReversal(line)) 
				{
					...
					else
					{
						// MZ Goodwill
						// if Shipment CostDetail exist then get Cost from Cost Detail
						String clientName = Env.getContext(getCtx(), "#AD_Client_Name");
						if(clientName.equals("RIZKY MITRA PANGAN")) {
							ProductCost pc = line.getProductCost();
							costs = pc.getProductCosts(as, line.getAD_Org_ID(), MCost.COSTINGMETHOD_StandardCosting, line.getC_OrderLine_ID(), true);
						}else	
							costs = line.getProductCosts(as, line.getAD_Org_ID(), true, "M_InOutLine_ID=?");
						// end MZ
					}
